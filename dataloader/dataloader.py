...
class Load_Dataset(Dataset):
    ...
    def __init__(self, dataset, config, training_mode):
        ...
        y1_train = dataset["labels_task300"]
        y2_train = dataset["labels_task450"]
        y3_train = dataset["labels_task4704"]
        ...
        if isinstance(X_train, np.ndarray):
            ...
            self.y1_data = torch.from_numpy(y1_train).long()
            self.y2_data = torch.from_numpy(y2_train).long()
            self.y3_data = torch.from_numpy(y3_train).long()
        else:
            self.x_data = X_train
            self.y1_data = y1_train
            self.y2_data = y2_train
            self.y3_data = y3_train
    ...
    def __getitem__(self, index):
        if self.training_mode == "self_supervised" or self.training_mode == "SupCon":
            return self.x_data[index], self.y1_data[index], self.y2_data[index], self.y3_data[index], self.aug1[index], self.aug2[index]
        else:
            return self.x_data[index], self.y1_data[index], self.y2_data[index], self.y3_data[index], self.x_data[index], self.x_data[index]
...
def data_generator(data_path, configs, training_mode):
    ...
    elif "_20p" in training_mode:
        train_dataset = torch.load(os.path.join(data_path, "train_20perc.pt"))
    ...
    '''
    _dataset返回self.x_data[index], self.y_data[index], self.aug1[index], self.aug2[index]
    '''
